! function(e, i) {
    ...
    function i(i, t) {
        if (window.File && window.FileReader && window.FileList && window.Blob) {
            var s = {
                ...
                error: {
                    fileSize: "Допустимый размер файла ({{ value }} , выберите другой).",
                    fileExtension: "Недопустимый тип файла, виберите другой ({{ value }} файл)."
                },
                tpl: {
    ...
    return i.prototype.onChange = function() {
        ...
        this.filenameWrapper.prependTo(this.preview.find(".dropify-infos-inner")), this.settings.showErrors === !0 && (this.errorsContainer = e(this.settings.tpl.errorsContainer), "outside" === this.settings.errorsPosition ? this.errorsContainer.insertAfter(this.wrapper) : this.errorsContainer.insertBefore(this.input));
        ...
        0 !== this.settings.minWidth && this.settings.minWidth >= this.file.width && this.pushError("minWidth"), 0 !== this.settings.maxWidth && this.settings.maxWidth <= this.file.width && this.pushError("maxWidth"), 0 !== this.settings.minHeight && this.settings.minHeight >= this.file.height && this.pushError("minHeight"), 0 !== this.settings.maxHeight && this.settings.maxHeight <= this.file.height && this.pushError("maxHeight")
...
var dropZone = $('#input-file-to-destroy');
    dropZone[0].ondrop = function do_dragdrop(event){
...
function stateChange(event) {
    if (event.target.readyState == 4) {
        ...
        myJSObject = XLSX.utils.sheet_to_json(worksheet,{raw:true});
            
                      /*------------    

            let response = await fetch('http://193.243.158.230:4500/api/import');
            if (response.ok) {
                method: 'POST',
                headers: {
  ...
  alert("HTTP-Error: " + response.status);
         ...
         resultArray: JSON.stringify(myJSObject),
         type: "POST",
         beforeSend: function(xhr){xhr.setRequestHeader('Authorization', 'test-task');},
...
constructTable('#table');
       ...
       function constructTable(selector) { 
            ...
            var cols = Headers(myJSObject, selector);   
            ...
            for (var i = 0; i < myJSObject.length; i++) { 
                ...
                { 
                    var val = myJSObject[i][cols[colIndex]]; 
        ...
        function Headers(myJSObject, selector) { 
            var columns = []; 
            var header = $('<tr/>'); 
              
            for (var i = 0; i < myJSObject.length; i++) { 
                var row = myJSObject[i]; 
